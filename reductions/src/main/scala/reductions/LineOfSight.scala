...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    upsweepSeqHelper(input, from, until, 0)
  }

  def upsweepSeqHelper(input: Array[Float], from: Int, until: Int, currMax: Float):Float = {
    if (from < until && from < input.length) {
      if (from == 0) upsweepSeqHelper(input, from + 1, until, max(currMax, 0))
      else upsweepSeqHelper(input, from + 1, until, max(currMax, input(from)/from))
    }
    else currMax
  }
