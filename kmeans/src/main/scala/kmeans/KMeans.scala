...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    means.map(mean => (mean, GenSeq[Point]())).toMap ++ points.groupBy(findClosest(_, means))
  }
  ...
  def getSquareDistance(p1: Point, p2: Point): Double = {
    Math.pow(p1.x - p2.x,2) + Math.pow(p1.y - p2.y, 2) + Math.pow(p1.z - p2.z, 2)
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans).map(pair => getSquareDistance(pair._1, pair._2)).find(x => x > eta) match {case Some(_) => false case _ => true}
  }

  @tailrec
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    //classify points
    val classifierMap = classify(points, means)
    // calculate new means
    val newMeans = update(classifierMap, means)
